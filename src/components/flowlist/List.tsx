...
// import { PageList } from 'api/mall'
// import EndTip from './EndTip'
// import assets from 'assets'
// import { NPLoading } from 'native-modules'

export interface FlowProps<ItemT> extends Omit<FlatListProps<ItemT>, 'renderItem'> {
  ...
  renderItem: (item: ItemT, index: number, total: number) => React.ReactElement<any>
}
...
class FlowList<ItemT> extends React.Component<FlowProps<ItemT>, IState<ItemT>> {
  static defaultProps = {
    ...
    LoadingComponent: () => (
      <View style={{ justifyContent: 'center', alignItems: 'center' }}>
        ...
        <Text style={{ fontSize: transformSize(28), marginTop: transformSize(10), color: '#ccc' }}>加载中…</Text>
      </View>
    ),
  }
  ...
  componentDidUpdate(preProps: FlowProps<ItemT>) {
    let update = ['api', 'pageSize'].some((key: string) => preProps[key] !== this.props[key])
    //'apiData' 浅比较
  ...
  private fetchData = async (refresh: boolean | void): Promise<void> => {
    ...
    try {
      ...
      // 	await httpCache(api, params, this.afterFetchData);
      // this.getCacheData(this.props.cacheKey)
      ...
      this.isLoading = false
      this._isMounted &&
        this.setState({ refreshing: false, noMoreData: true, hasLoadData: true }, () => {
          this.finishRefresh()
        })
    }
  }
  ...
  private afterFetchData = (res: PageList<ItemT>) => {
    ...
    let { pageSize, disabledPage, onFetchedData, noMoreForLessPageSize } = this.props
    let { data, noMoreData } = this.state

    let resEntities = res && res.entities
    let fetchData = resEntities && onFetchedData ? onFetchedData(res.entities, res) : null
  ...
  finishRefresh = () => {
    if (this.props.refreshScrollTop) {
      let scroll = this.props.useAnimated ? (this.flatListRef.current as any)._component : this.flatListRef.current
  ...
  render() {
    ...
    let { disabledRefresh, onRefresh, onEndReached, useAnimated, keyExtractor, ...otherProps } = this.props
  ...
  private renderRefreshControl = () => {
    ...
    refreshControl = refreshControl || <RefreshControl refreshing={refreshing} title="下拉获取更多内容" />
  ...
  private renderListFooterComponent = () => {
    ...
    return <EndTip noMoreData={noMoreData} endTipStyle={this.props.endTipStyle} />
  }
}
